...
constexpr uint8_t RMD_CMD_MOTOR_OFF       = 0x80;  // Motor off / clear error
constexpr uint8_t RMD_CMD_MOTOR_ON        = 0x88;  // Motor on (enable)
...
constexpr uint8_t RMD_CMD_READ_MULTI_TURN = 0x60;  // Read multi-turn output angle
...
public:
     ...
     */
    /** Build 0x80 "Motor Off" command (also clears error state). */
    static void buildMotorOff(uint8_t data[8]);

    /** Build 0x88 "Motor On" command. */
    static void buildMotorOn(uint8_t data[8]);

    static void buildReadStatus2(uint8_t data[8]);

    /**
     * Build 0x60 "Read Multi-Turn Angle" command.
     * Response carries int64 (data[1..7] with 56-bit signed) at 0.01°/LSB on the
     * OUTPUT shaft (post-gearbox), so it survives multiple revolutions without
     * wrap. Use this to override the single-turn encoder field of 0x9C/0xA4.
     */
    static void buildReadMultiTurn(uint8_t data[8]);
    ...
    /**
     * Parse 0x60 multi-turn angle response.
     * data[1..7] = signed 56-bit value at 0.01°/LSB on the output shaft.
     * Only `position_rad` and `valid` are populated; velocity/effort/temperature
     * fields stay at default 0 (caller must keep using 0x9C/0xA4 for those).
     */
    static MotorState parseMultiTurn(const uint8_t rx_data[8], uint8_t rx_len);
};
