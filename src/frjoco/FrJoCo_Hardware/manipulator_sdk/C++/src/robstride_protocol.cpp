...
void RobstrideProtocol::buildMotionControl(uint32_t& arb_id, uint8_t data[8],
                                           uint8_t motor_id,
                                           ...
                                           float vel_min, float vel_max)
{
    // comm_type=1: torque_raw goes into data1 field (bits 23~8) of arb_id
    uint16_t torque_raw = floatToUint16(torque, torque_min, torque_max);
    arb_id = makeArbId(RS_MSG_CONTROL, torque_raw, motor_id);

    uint16_t pos_raw = floatToUint16(position, RS_ANGLE_MIN, RS_ANGLE_MAX);
    uint16_t vel_raw = floatToUint16(velocity, vel_min, vel_max);
    uint16_t kp_raw  = floatToUint16(kp,  0.0f, 500.0f);
    uint16_t kd_raw  = floatToUint16(kd,  0.0f, 5.0f);

    data[0] = (pos_raw >> 8) & 0xFF;
    data[1] =  pos_raw       & 0xFF;
    data[2] = (vel_raw >> 8) & 0xFF;
    data[3] =  vel_raw       & 0xFF;
